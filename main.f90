Program advection
!*******************************************************
! 二维浅水方程，考虑了对流项，风应力，底摩擦，水平压强梯度力，干湿
! 网格算法。
! 本程序为主程序（main.f90），此外，还有变量声明模块（var.f90）、
! 计算过程模块（proc.f90）以及水深文件topo.dat
...
Do n = 1, nt
  ...
  ! 迭代变量，以第n+1步的计算结果替换第n步的eta，h，mask，u，v，B
  Do j = 0, ny+1
    Do i = 0, nx+1
